...
export async function Log(
  ...
  try {
    const accessToken = process.env.ACCESS_TOKEN || process.env.NEXT_PUBLIC_ACCESS_TOKEN;
    if (!accessToken) {
      // Missing token, but we must fail silently per requirements.
      return;
    }
    ...
    // Handle all failures silently — never throw or crash.
  }
}
